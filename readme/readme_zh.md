...
- **基于标签和提示词的全文搜索** —— 由 SQLite FTS5 驱动,
  ...
  "sunlight"。
- **互斥的颜色分组** —— 通过有序的关键词对规则进行定义;
  每张图片会被归入*第一条*匹配的规则,并渲染在带颜色的
  可折叠容器中。这是 Artifex 的招牌功能,可按主题、角色或
  风格,在视觉上一目了然地组织上千张生成图。
...
Artifex 的招牌组织功能。通过有序的**关键词对**进行定义 ——
每一对指定一组关键词,图片必须**全部命中**才能归入该分组。
图片会被归入**第一条**匹配的关键词对,因此分组天然互斥;
未命中任何规则的图片会落入 "Other"。

每个分组会被分配独立的颜色,并以可折叠容器的形式呈现,
在视觉上将不同主题、角色或风格清晰区分开。

- **基于标签的分组** —— 按图片的标签(自动生成或自定义)匹配
- **基于提示词的分组** —— 按 ComfyUI 元数据中提取出的
  正向和负向提示词文本匹配
